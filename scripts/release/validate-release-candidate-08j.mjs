...
if(pkg.version!=="0.21.0"||model.version!==pkg.version||matrix.version!==pkg.version||modules.version!==pkg.version)throw new Error("Versões 08J divergentes.");
if((modules.modules||[]).length!==22||(modules.modules||[]).some(item=>item.status!=="active"))throw new Error("O 08J deve preservar 22 módulos ativos.");
if((roles.permissions||[]).length!==117)throw new Error(`Permissões alteradas sem decisão: ${(roles.permissions||[]).length}`);
