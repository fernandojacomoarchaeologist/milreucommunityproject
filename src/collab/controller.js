...
function createDemoDeploymentWorkspace(homologationModel,deploymentProfile,deploymentReadiness,master=false){
  ...
  const run={
    ...
    version:"0.37.0",commit_sha:null,status:master?"in-progress":"planned",
...
function createDemoOperationalWorkspace(model,retentionModel,runtime,master=false){
  ...
  const operationalRuns=[
    {id:"demo-operational-run",environment:"local",version:"0.37.0",commitSha:null,status:"running",summary:null,startedAt:iso(-2*3600000),completedAt:null}
  ];
...
class CollaborativeController{
  ...
  async generateMuseumReviewSnapshot(cycleId,version="0.37.0"){
