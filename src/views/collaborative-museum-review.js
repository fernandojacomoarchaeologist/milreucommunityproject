...
export function collaborativeLibraryView(context,filters={}){
  ...
  return collaborativeShell(context,"/area-colaborativa/biblioteca",`
    ...
    <div class="collab-library-grid">${resources.length?resources.map(item=>`<article><span>${esc(item.category)}</span><h2>${esc(item.title)}</h2><p>${esc(item.path)}</p><a href="#/area-colaborativa/biblioteca/${esc(item.code)}">Abrir recurso</a></article>`).join(""):`<p class="collab-empty-line">Nenhum recurso corresponde aos filtros.</p>`}</div>
  `);
}
...
function reviewCard(context,row,management=false){
  ...
  return `<article class="museum-review-card museum-review-card--${esc(row.status)}" data-review-card data-status="${esc(row.status)}" data-search="${esc(`${row.memory_id} ${title}`.toLowerCase())}">
    <div class="museum-review-card__image">${memory?.media?.variants?.thumbnail?`<img src="${assetUrl(memory.media.variants.thumbnail)}" alt="">`:""}</div>
...
export function collaborativeMuseumReviewManagementView(context,section="overview"){
  ...
  if(section==="releases"){
    return collaborativeShell(context,"/area-colaborativa/gestao/revisao-museu/releases",`
      ...
      ${cycle&&hasPermission(context,"museum.review.export")?`<form class="collab-form compact-form snapshot-generator" data-museum-snapshot-form data-cycle-id="${esc(cycle.id)}"><label>Versão<input name="version" value="0.21.0" required></label><button type="submit">Gerar snapshot validado</button><p data-collab-feedback></p></form>`:""}
