...
export function collaborativeMuseumReviewManagementView(context,section="overview"){
  ...
  if(section==="releases"){
    return collaborativeShell(context,"/area-colaborativa/gestao/revisao-museu/releases",`
      ...
      ${cycle&&hasPermission(context,"museum.review.export")?`<form class="collab-form compact-form snapshot-generator" data-museum-snapshot-form data-cycle-id="${esc(cycle.id)}"><label>Versão<input name="version" value="0.20.0" required></label><button type="submit">Gerar snapshot validado</button><p data-collab-feedback></p></form>`:""}
