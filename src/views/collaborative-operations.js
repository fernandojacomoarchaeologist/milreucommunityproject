  ...
  return `<article class="operations-run operations-run--${esc(run.status)}">
    ...
    <div class="operations-check-list">${results.map(result=>{
      ...
      return `<form class="operations-check operations-check--${esc(result.status)}" data-operation-result-form data-run-id="${esc(run.id)}" data-check-code="${esc(result.checkCode)}">
        ...
        <select name="status">${optionList(context.operationalGovernanceModel?.checkStatuses||[],result.status)}</select>
        <input name="evidenceReference" value="${esc(result.evidenceReference||"")}" placeholder="${check.evidence_required?"Evidência obrigatória":"Referência de evidência"}">
        <input name="notes" value="${esc(result.notes||"")}" placeholder="Notas">
