...
test("modelo contém vinte eventos únicos",()=>{assert.equal(model.eventTypes.length,20);assert.equal(new Set(model.eventTypes.map(x=>x.code)).size,20);});
test("dez categorias cobrem o projeto",()=>{assert.equal(model.categories.length,10);for(const code of["membership","tasks","contributions","museum-review","withdrawals","operations"])assert.ok(model.categories.some(x=>x.code===code));});
