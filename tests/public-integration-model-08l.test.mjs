...
test("modelo público é staging/snapshots-only e sem efeitos ativos",()=>{assert.equal(pub.version,"0.39.0");assert.equal(pub.publicReadsSnapshotsOnly,true);assert.equal(pub.activePublicEffectsByDefault,0);assert.equal(pub.productionApproval,"blocked");});
