...
test("modelo público é staging/snapshots-only e sem efeitos ativos",()=>{assert.equal(pub.version,"0.37.1");assert.equal(pub.publicReadsSnapshotsOnly,true);assert.equal(pub.activePublicEffectsByDefault,0);assert.equal(pub.productionApproval,"blocked");});
...
test("catálogo tem 140 permissões e as 13 novas",()=>{assert.equal(roles.permissions.length,152);for(const p of ["participation.view","public-integration.propose","public-integration.activate","evolution.decide"])assert.ok(roles.permissions.includes(p));});
