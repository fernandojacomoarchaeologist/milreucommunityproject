...
test("08J mantém versão e candidata",()=>{assert.equal(model.version,"0.22.0");assert.equal(model.package,"08J");assert.equal(model.candidate,"RC1");});
test("08J preserva módulos e permissões",()=>{assert.equal(modules.modules.length,23);assert.ok(modules.modules.every(item=>item.status==="active"));assert.equal(roles.permissions.length,127);});
